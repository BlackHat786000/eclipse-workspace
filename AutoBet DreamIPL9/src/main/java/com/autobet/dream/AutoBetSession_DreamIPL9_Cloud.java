...
public class AutoBetSession_DreamIPL9_Cloud {
	
	public static void send_mail(String sname, String bet, int runs, int odds, String amount) {
		
		try {
			Email email = new Email("autobet786@gmail.com", "bhajjiterminator");
			email.setFrom("autobet786@gmail.com", "AutoBet Notification - "+bet);
			email.setSubject(sname);
			email.setContent("<h1>This is to inform you that your "+bet+" has been successfully placed on "+runs+" RUNS @ "+odds+" for amount "+amount+" Rs.</h1>", "text/html");
	...
	public static void main(String[] args) {
		...
		System.out.println("******************************** Welcome To Punter's AutoBetSession v3.0 - set the session both sides as you want ********************************\n\n");
		...
		if(not.equals("Ball Running") || not.equals("SUSPENDED")) {
			...
			case "Y" : if (yes_runs <= yes_target && yes_odds == 100) {
				...
				String toast_message = wait.until(ExpectedConditions.elementToBeClickable(By.className("jq-toast-single"))).getText();
					...
					AutoBetSession_DreamIPL9_Cloud.send_mail(sname, "YES BET", yes_runs, yes_odds, amount);
					System.out.println("\n-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------");
					++monitor;
					mode = "N";
			...
			case "N" : if (not_runs >= not_target && not_odds == 100) {
				...
				String toast_message = wait.until(ExpectedConditions.elementToBeClickable(By.className("jq-toast-single"))).getText();
					...
					driver.navigate().refresh();
					System.out.println(toast_message);
					AutoBetSession_DreamIPL9_Cloud.send_mail(sname, "NOT BET", not_runs, not_odds, amount);
					System.out.println("\n-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------");
					++monitor;
					mode = "Y";
